...
- (void)viewDidLoad {
    ...
    // For each website in the websites array insert an NSNull into the icons array
    // These NSNulls will be replaced when the images are loaded from the network
    for (__unused NSString *website in websites){
        [websiteIcons addObject:[NSNull null]];
    }
    
    // Get a new operation queue
    NSOperationQueue *backgroundQueue = [[NSOperationQueue alloc] init];
    int websiteNumber = 0; // for keeping track of which index to insert the new image to
    for (NSString *website in websites){
        [backgroundQueue addOperationWithBlock:^{
        
            // Construct a URL for the website's icon
            NSURL *iconURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://www.%@/favicon.ico", website]];
            ...
            if (loadedData != nil){
                ...
                // If it was, insert the image into the table view on the main queue
