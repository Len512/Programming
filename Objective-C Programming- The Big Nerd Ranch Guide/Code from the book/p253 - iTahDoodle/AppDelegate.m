...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // Add here everything that needs to be done before the user interacts with the application
    // Setting up views programmatically (create, configure, put views on the screen)
    ...
    // Add our three UI elements to the window's view controller
...
- (NSInteger) tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    // Because this table view only has one section, the number of rows in it is
    // equal to the number of items in the tasks array
    return [self.tasks count];
}
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    // To improve performance, this method first checks for an existing cell object
    // that we can reuse. If there isn't one, then a new cell is created
