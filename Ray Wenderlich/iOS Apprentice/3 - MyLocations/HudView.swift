...
// Show a quick animation (Heads-Up Display or HUD) when the user is Done tagging a location
...
class HudView: UIView {
    ...
    func show(animated: Bool) {
        if animated {
            ...
            // Set up an animation and describe it with a closure/block
            UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.5, options: [], animations: {
                // Set up the new state after the animation completes
                self.alpha = 1
