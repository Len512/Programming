...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        customiseAppearance()
        // Pass the NSManagedObjectContext to the CurrentLocationViewController
        let tabBarController = window!.rootViewController as! UITabBarController
        if let tabBarViewControllers = tabBarController.viewControllers {
            let currentLocationViewController = tabBarViewControllers[0] as! CurrentLocationViewController
            currentLocationViewController.managedObjectContext = managedObjectContext
            // Look up the LocationsViewController in the storyboard and give it a reference to the context
    ...
    // MARK - Handle error methods
    func listenForFatalCoreDataNotifications() {
        // Get notified whenever a MyManagedObjectContextSaveDidFailNotification is posted
    ...
    // To show the alert we need a view controller that is currently visible
